process.process_characteristics_template <- function(viz = as.viz("process_characteristics_template")) {
  ...
  boundaries <- dplyr::select(deps[["process_watershed_map_data"]]$hu_boundary, huc12, areasqkm, name)
  ...
  for(hu in names(nldi_chars)) {
    ...
    nldi_chars[[hu]][["drainage_area"]] <- round((boundaries[hu,"areasqkm"] * 0.386102))
